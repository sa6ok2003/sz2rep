...
def reg_user(id,ref,utm):
    ...
    if sql.fetchone() is None:
        ...
        sql.execute(f"INSERT INTO trafik VALUES (?,?,?)", ('linl_ferst', 'https://t.me/FoxRelease', 100))
        db.commit()
...
def cheak_traf():
    ...
    c4 = sql.execute(f"SELECT parametr FROM trafik WHERE chanel = 'linl_ferst'").fetchone()[0]
    list = [c1,c2,c3,c4]
    return list
...
def obnovalinl(link):
    ...
    sql.execute(f"UPDATE trafik SET parametr= '{link}' WHERE chanel = 'linl_ferst'")
    db.commit()

...
import sqlite3
from .sqlit import info_members, reg_one_channel, reg_channels,del_one_channel,cheak_traf,obnovatrafika,info_chyornaya_vdova,info_good_film1,info_films_online_everyday,reg_partners_schet,cheach_all_par,info,cheak_zakup,reg_utm,obnovalinl
...
ADMIN_ID =[ADMIN_ID_1,MODERN_ID_5]
...
async def admin_ka(message: types.Message):
    ...
    if id in ADMIN_ID:
        ...
        bat_ferst = types.InlineKeyboardButton(text='Настройка последнего канала', callback_data='sett')
...
# НАСТРОЙКА Последнего канала
@dp.callback_query_handler(text='sett')
async def baza12131202(call: types.callback_query):
    markup = types.InlineKeyboardMarkup()
    bat_a = types.InlineKeyboardButton(text='ОТМЕНА', callback_data='otemena')
    markup.add(bat_a)

    await bot.send_message(call.message.chat.id, text=f'Введите полную ссылку на последний канал. Пример:\n\n'
                                                      f'https://t.me/FoxRelease',reply_markup=markup,disable_web_page_preview=True)
...
async def link_obnovlenie(message: types.Message, state: FSMContext):
    mas = message.text
    try:
        obnovalinl(mas)  # Внесение новой последней ссылке
        obnovlenie()
        await bot.send_message(message.chat.id, text=f'Успешно!')

    except:
        await bot.send_message(message.chat.id, text=f'Неудача :((')
    await state.finish()
